...
app.put("/books/updateBook", authenticateUser, authorizeUser, (req, res) => {
  ...
  if (
    ...
    // If the title, author, genre, or index is not a string or number, or if the index is out of range,
    // send a 400 Bad Request status code and an error message
    res.status(400).send({
      message: "failure",
      error:
...
// Here's an example of how middleware functions can be used to protect routes in an Express.js application:
